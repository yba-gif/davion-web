...
9. **No em-dashes.** Founder preference (2026-05-21). The em-dash character (U+2014, the long dash) is banned site-wide. Use a comma for parenthetical asides, a colon for list-item labels (`Ingest: transactions, KYC`), or split into two sentences with a period for breaks. The em-dash is a literary tic; Davion writes for procurement officers, not essayists. Enforced by `pnpm lint:dashes`.
   - Good: *"AlpOS unifies data, ontology, and analytics, governed end to end."*
   - Good: *"AlpOS unifies data, ontology, and analytics. Governed end to end."*
   - (An en-dash `–` in a number range like `6–12 weeks` is allowed; it's the em-dash specifically that's banned.)
